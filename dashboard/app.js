...
const I18N = {
  zh: {
    ...
    detection_title: '偵測事件 ID 參考', detection_desc: '各攻擊手法對應的 Windows Event ID，點擊 Event ID 可複製',
  ...
  en: {
    ...
    detection_title: 'Detection Event IDs', detection_desc: 'Windows Event IDs per attack technique — click to copy',
  ...
  ja: {
    ...
    detection_title: 'イベントID検知リファレンス', detection_desc: '攻撃手法対応の Windows Event ID — クリックでコピー',
    ...
    severity_critical: '深刻 (Critical)', severity_high: '高危 (High)', severity_medium: '中危 (Medium)',
...
function applyI18n() {
  ...
  const defenseOverall = document.querySelector('#tab-defense .fw-600.text-primary');
  if (defenseOverall) defenseOverall.textContent = t('defense_overall');
...
function toggleChecklist(key, element) {
  ...
  if (section) {
    ...
    if (progressEl) progressEl.textContent = `${checkedCount} / ${total} 完成`;
